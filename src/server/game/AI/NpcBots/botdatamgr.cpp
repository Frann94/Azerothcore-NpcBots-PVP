...
// Fran: handle specific class/spec for arena/bg
bool isClassForced = false;
int classPairNumber;
bool isSpecForced = false;
uint8 specNumber;
// Fran: PVP Team formation
bool forcedTeamTemplate = true;
int totalHealer;
int totalTank;
int totalDps;
    ...
    bool Execute(uint64 e_time, uint32 /*p_time*/) override
    {
        ...
        else if (Creature const* bot = BotDataMgr::FindBot(_botGUID.GetEntry()))
        {
            ...
            if (bgPlayer && bgPlayer->IsInWorld() && bgPlayer->InBattleground() && bgPlayer->GetMap()->IsBattlegroundOrArena())
            {
                ...
                // Fran: handle logic for manual skirmish
                BattlegroundQueue& queue = sBattlegroundMgr->GetBattlegroundQueue(_bgQueueTypeId);
                if (!queue.isSkirmishCommand && !queue.IsBotInvited(_botGUID, bg->GetInstanceID()))
                {
                    AbortMe();
                    return true;
                }

                if (!queue.isSkirmishCommand) {
...
private:
        ...
        bool immediate, PvPDifficultyEntry const* bracketEntry, NpcBotRegistry* registry)
    {
        ...
        uint8 bot_class = spareBotPair.first;
        ...
        else if (totalHealer > 0 && bot_ai::IsHealingClass(bot_class)) {
            bot_spec = bot_ai::GetHealingSpec(bot_class);
            bot_ai::SetForcedSpec(bot_spec, true);
            totalHealer--;
        }
        else if (totalTank > 0 && bot_ai::IsTankingClass(bot_class)) {
            bot_spec = bot_ai::GetTankingSpec(bot_class);
            bot_ai::SetForcedSpec(bot_spec, true);
            totalTank--;
        }
        else if (totalDps > 0) {
            bot_spec = bot_ai::GetDpsSpec(bot_class);
            bot_ai::SetForcedSpec(bot_spec, true);
            totalDps--;
        }
        else {
            bot_spec = bot_ai::SelectSpecForClass(bot_class, false);
        }
...
public:
    ...
    bool GenerateWanderingBotsToSpawn(uint32 count, int32 map_id, int32 team, bool immediate, PvPDifficultyEntry const* bracketEntry, NpcBotRegistry* registry, uint32& spawned)
    {
        ...
        else
        {
            ...
            // Fran: force specific classes
            if (isClassForced) {
                isClassForced = false;
                if (_spareBotIdsPerClassMap.size() >= classPairNumber) {
                    auto classPair = std::next(_spareBotIdsPerClassMap.begin(), classPairNumber-1);

                    // Extract class and set of bot IDs from the first pair
                    uint8 botClass = classPair->first;
                    std::set<uint32> botIds = classPair->second;

                    // Iterate over the bot IDs
                    for (uint32 spareBotId : botIds) {
                        // Retrieve extras for the current bot ID
                        NpcBotExtras const* orig_extras = ASSERT_NOTNULL(BotDataMgr::SelectNpcBotExtras(spareBotId));

                        // Determine bot faction and team
                        uint32 bot_faction = GetDefaultFactionForRaceClass(botClass, orig_extras->race);
                        uint32 botTeam = BotDataMgr::GetTeamForFaction(bot_faction);

                        // Check if botTeam matches the desired team
                        if (int32(botTeam) == team) {
                            // Check if there's a bracket entry and bot level is within the bracket
                            if (!bracketEntry || (BotDataMgr::GetMinLevelForBotClass(botClass) <= bracketEntry->maxLevel)) {
                                // Insert the bot ID into teamSpareBotIdsPerClass
                                teamSpareBotIdsPerClass[botClass].insert(spareBotId);
                            }
                        }
                    }
                }
    ...
    void setForcedClass(int botClass, uint8 botSpec) {
        isClassForced = true;
        classPairNumber = botClass;
        isSpecForced = true;
        specNumber = botSpec;
    }

    void setGearChances(uint16 wrathfull, uint16 relentless) {
        bot_ai::setGearChances(wrathfull, relentless);
    }
};
...
bool BotDataMgr::GenerateBattlegroundBots(Player const* groupLeader, [[maybe_unused]] Group const* group, BattlegroundQueue* queue, PvPDifficultyEntry const* bracketEntry, GroupQueueInfo const* gqinfo)
{
    ...
    forcedTeamTemplate = true;
    if (needed_bots_count_a)
    {
        // Fran: Set team formation
        switch (bgTypeId) {
                ...
                switch (proto.InventoryType)
                {
                    ...
                    case INVTYPE_SHIELD:
                        if (proto.Armor == 0)
                            break;
                        if (!is_caster_item)
                            push_gear_to_classes(proto, BOT_SLOT_OFFHAND, reqLstep, { BOT_CLASS_WARRIOR, BOT_CLASS_PALADIN });
