...
enum rbac
{
    ...
    RBAC_PERM_COMMAND_NPCBOT_GROUP = SEC_GAMEMASTER
};
//end Acore only
#endif
...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        static ChatCommandTable npcbotCommandTable =
        {
            ...
            { "resetcd",    HandleNpcBotResetCooldownsCommand,      rbac::RBAC_PERM_COMMAND_NPCBOT_GROUP,              Console::No  },
        };
    ...
    static bool HandleNpcBotSpawnCommand(ChatHandler* handler, Optional<std::string> creVal)
    {
        ...
        uint8 bot_spec = bot_ai::SelectSpecForClass(_botExtras->bclass, false);
    ...
    static bool HandleNpcBotAddCommand(ChatHandler* handler)
    {
        ...
        if (owner->GetBotMgr()->AddBot(bot, false) == BOT_ADD_SUCCESS)
        {
    ...
    static bool HandleNpcBotResetCooldownsCommand(ChatHandler* handler) {
        Player* player = handler->GetSession()->GetPlayer();
        ...
        for (BotMap::const_iterator citr = bmap->begin(); citr != bmap->end(); ++citr)
        {
            ...
            if (!bot)
                continue;
            bot_ai::BotSpellMap spells;
            spells = bot->GetBotAI()->GetSpellMap();
            for (bot_ai::BotSpellMap::const_iterator itr = spells.begin(); itr != spells.end(); ++itr)
            {
                itr->second->cooldown = 0;
            }
            bot->RemoveAllAurasOnDeath();
        }
        handler->SendSysMessage("Bot CDs reset");
        return true;
    }
