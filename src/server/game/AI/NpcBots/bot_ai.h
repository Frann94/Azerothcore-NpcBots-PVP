...
class bot_ai : public CreatureAI
{
    public:
        ...
        uint8 GetSpec() const;
        static uint8 GetHealingSpec(uint8 m_class);
        static uint8 GetTankingSpec(uint8 m_class);
        static uint8 GetDpsSpec(uint8 m_class);
        static void SetForcedSpec(uint8 spec, bool isForcedSpec);
        static uint8 SelectSpecForClass(uint8 m_class, bool isForcedSpec);
        static uint32 TextForSpec(uint8 spec);
        static bool IsValidSpecForClass(uint8 m_class, uint8 spec);
        static void setGearChances(uint16 wrathfull, uint16 relentless);
