...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    me->SetUnitFlag2(UnitFlags2(CREATURE_FLAG_EXTRA_ALL_DIMINISH));
...
void bot_ai::CheckOwnerExpiry()
{
    ...
    if (timeNow >= baseTimeStamp + expireTime)
    {
        ...
        //...spec
        uint8 spec = SelectSpecForClass(npcBotExtra->bclass, false);
...
void bot_ai::SetBotAwaitState(uint8 state)
{
    ...
    Events.AddEvent(awaitStateRemEvent, Events.CalculateTime(60000));
}
...
void bot_ai::BuffAndHealGroup(uint32 diff)
{
    ...
    if (IAmFree())
    {
        if (!HasRole(BOT_ROLE_HEAL) && Rand() > 40)
            return;
        ...
        if (me->GetMap()->IsBattleArena()) {
            GetNearbyFriendlyTargetsList(targets2, 100);
        }
        else {
            GetNearbyFriendlyTargetsList(targets2, 50);
        }
...
bool bot_ai::IsInBotParty(Unit const* unit) const
{
    ...
    if (IAmFree())
    {
        ...
        //cheap check
        if (Group const* gr = me->GetBotGroup())
        {
            //group member case
...
bool bot_ai::CanBotAttack(Unit const* target, int8 byspell, bool secondary) const
{
    if (!target)
        return false;
    if (target->IsCreatedByPlayer() || target->IsPet() || target->IsCritter() || target->IsNPCBotPet())
        return false;
...
void bot_ai::InitSpec()
{
    uint8 spec;
    if (IAmFree())
        spec = SelectSpecForClass(_botclass, isForcedSpec);
    else
    {
        NpcBotData const* npcBotData = BotDataMgr::SelectNpcBotData(me->GetEntry());
...
void bot_ai::SetForcedSpec(uint8 spec, bool forcedSpec) {
    specNumber = spec;
    isForcedSpec = forcedSpec;
}

uint8 bot_ai::GetHealingSpec(uint8 m_class) {
...
uint8 bot_ai::GetTankingSpec(uint8 m_class) {
    std::vector<uint8> specs;
    specs.reserve(3);
    switch (m_class)
    {
...
uint8 bot_ai::GetDpsSpec(uint8 m_class) {
    std::vector<uint8> specs;
    specs.reserve(3);
    switch (m_class)
    {
...
uint8 bot_ai::SelectSpecForClass(uint8 m_class, bool forcedSpec)
{
    if (forcedSpec && specNumber != NULL) {
        uint8 spec = specNumber;
        specNumber = NULL;
        return spec;
    }
    else {
        ...
        switch (m_class)
        {
            ...
            case BOT_CLASS_DEATH_KNIGHT: //any
                ...
                specs.push_back(BOT_SPEC_DK_UNHOLY);
                break;
            case BOT_CLASS_SHAMAN: //any
                specs.push_back(BOT_SPEC_SHAMAN_ELEMENTAL);
                specs.push_back(BOT_SPEC_SHAMAN_ENHANCEMENT);
                specs.push_back(BOT_SPEC_SHAMAN_RESTORATION);
                break;
            case BOT_CLASS_MAGE: //any
                specs.push_back(BOT_SPEC_MAGE_ARCANE);
                specs.push_back(BOT_SPEC_MAGE_FIRE);
                specs.push_back(BOT_SPEC_MAGE_FROST);
                break;
...
void bot_ai::setGearChances(uint16 wrathfull, uint16 relentless) {
    wrathfulChance = wrathfull;
    relentlessChance = relentless;
}
...
void bot_ai::InitEquips()
{
    ...
    if (IsWanderer())
    {
        ...
        // Fran: Set ilvl for all items at random
        uint32 minItemLevel = 220;
        uint32 maxItemLevel;
        /*
        238 Furious honor weapons
        232-238 Furious armor

        245 Relentless honor
        245-251 Relentless armor

        252 Furious arena weapons

        258 Relentless arena weapons

        264 Wrathful honor weapons
        277 Wrathful arena weapons
        264-270 Wrathful armor
        */
        if (Rand() < wrathfulChance) {
            minItemLevel = 264;
            maxItemLevel = 284;
        }
        else {
            if (Rand() < relentlessChance) {
                minItemLevel = 244;
                maxItemLevel = 263;
            }
            else {
        ...
        for (uint8 i = BOT_SLOT_MAINHAND; i < BOT_INVENTORY_SIZE; ++i)
        {
            ...
            Item* item = BotDataMgr::GenerateWanderingBotItem(i, _botclass, lvl,
                [this, lslot = i, minItemLevel, maxItemLevel](ItemTemplate const* proto) {
                if (!_canEquip(proto, lslot, true))
                    return false;

                // Fran: Force S8 medallion trinket
                bool useResilenceGear = me->GetLevel() >= DEFAULT_MAX_LEVEL && me->GetMap()->IsBattlegroundOrArena();
                if (lslot == BOT_SLOT_TRINKET1 && useResilenceGear && proto->ItemLevel == 264)
                {
                    switch (lslot)
                    {
                ...
                if ((proto->ItemLevel < minItemLevel || proto->ItemLevel > maxItemLevel) && lslot != BOT_SLOT_TRINKET2) {
                    return false;
                }

                // Fran: Force resilence gear
                if (useResilenceGear)
                {
                    switch (lslot)
                    {
                ...
                switch (GetSpec())
                {
                        ...
                        switch (lslot)
                        {
                            case BOT_SLOT_MAINHAND:
                                if (!(proto->InventoryType == INVTYPE_2HWEAPON))
                                    return false;
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return (stat.ItemStatType == ITEM_MOD_STRENGTH || stat.ItemStatType == ITEM_MOD_ATTACK_POWER) && stat.ItemStatValue > 0;
                                });
                            case BOT_SLOT_TRINKET2:
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return stat.ItemStatType == ITEM_MOD_ATTACK_POWER && stat.ItemStatValue > 0;
                                });
                            default:
                                break;
                        }
                        ...
                        switch (lslot)
                        {
                            ...
                            case BOT_SLOT_TRINKET2:
                                if (me->GetLevel() < 70)
                                    break;
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return stat.ItemStatType == ITEM_MOD_MANA_REGENERATION && stat.ItemStatValue > 0;
                                    });
                            default:
                                if (me->GetLevel() < 70)
                                    break;
                        ...
                        switch (lslot)
                        {
                            case BOT_SLOT_MAINHAND:
                                if (!(proto->InventoryType == INVTYPE_2HWEAPON))
                                    return false;
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return (stat.ItemStatType == ITEM_MOD_AGILITY || stat.ItemStatType == ITEM_MOD_ATTACK_POWER) && stat.ItemStatValue > 0;
                                });
                            case BOT_SLOT_TRINKET2:
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return stat.ItemStatType == ITEM_MOD_ATTACK_POWER && stat.ItemStatValue > 0;
                    ...
                    case BOT_SPEC_ROGUE_SUBTLETY:
                        switch (lslot)
                        {
                            ...
                            case BOT_SLOT_RANGED:
                                return proto->SubClass == ITEM_SUBCLASS_WEAPON_THROWN;
                            case BOT_SLOT_TRINKET2:
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return stat.ItemStatType == ITEM_MOD_ATTACK_POWER && stat.ItemStatValue > 0;
                                });
                    ...
                    case BOT_SPEC_ROGUE_COMBAT:
                        switch (lslot)
                        {
                            case BOT_SLOT_MAINHAND:
                            case BOT_SLOT_OFFHAND:
                                if (!(proto->SubClass == ITEM_SUBCLASS_WEAPON_SWORD || proto->SubClass == ITEM_SUBCLASS_WEAPON_AXE))
                                    return false;
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return (stat.ItemStatType == ITEM_MOD_AGILITY || stat.ItemStatType == ITEM_MOD_ATTACK_POWER) && stat.ItemStatValue > 0;
                                });
                    ...
                    case BOT_SPEC_DRUID_BALANCE:
                        switch (lslot)
                        {
                            case BOT_SLOT_TRINKET2:
                                return std::any_of(std::cbegin(proto->ItemStat), std::cend(proto->ItemStat), [](_ItemStat const& stat) {
                                    return stat.ItemStatType == ITEM_MOD_SPELL_POWER && stat.ItemStatValue > 0;
                                });
                            default:
                                if (me->GetLevel() < 70)
                                    break;
