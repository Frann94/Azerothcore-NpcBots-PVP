...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    // Ornfelt: always nagrand skirm // Fran: allow all arenas
    if (originalBgTypeId == 6) {
        std::vector<BattlegroundTypeId> arenas;
        arenas.push_back(BATTLEGROUND_NA);
        //arenas.push_back(BATTLEGROUND_BE);
        //arenas.push_back(BATTLEGROUND_RL);
        //arenas.push_back(BATTLEGROUND_DS);
        //arenas.push_back(BATTLEGROUND_RV);
        bgTypeId = Acore::Containers::SelectRandomContainerElement(arenas);
    }
