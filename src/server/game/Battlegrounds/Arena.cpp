...
Arena::Arena()
{
    ...
    StartMessageIds[BG_STARTING_EVENT_FIRST] = ARENA_TEXT_START_FIFTEEN_SECONDS;
...
void Arena::RemovePlayerAtLeave(Player* player)
{
    ...
    player->setTeamId(player->TeamIdForRace(player->GetRace()));
    ChrRacesEntry const* DBCRace = sChrRacesStore.LookupEntry(player->GetRace());
