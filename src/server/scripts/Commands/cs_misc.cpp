...
#include <Arena.h>
#include <Arena.cpp>
#include <AI/NpcBots/botdatamgr.cpp>
#include <iostream>
#include <string>
...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        static ChatCommandTable commandTable =
        {
            ...
            { "botgear",           HandleBotGearCommand,            SEC_ADMINISTRATOR,      Console::No  }
        };

        return commandTable;
    }

    static bool HandleBotGearCommand(ChatHandler* handler, std::vector<std::string_view> args)
    {
        auto tokens = args;
        if (args.empty() || !tokens.size())
        {
            handler->PSendSysMessage("Usage: .botgear [Wrathful%] [Relentless%]");
            handler->SetSentErrorMessage(true);
            return false;
        }
        auto tokensItr = tokens.begin();

        uint16 wrathfulChance = 10;
        uint16 relentlessChance = 30;

        if (tokensItr != tokens.end())
        {
            std::string_view wrathfulStr = *(tokensItr++);
            try
            {
                unsigned long wrathfulUL = std::stoul(std::string(wrathfulStr));
                wrathfulChance = static_cast<uint16>(wrathfulUL);
            }
        ...
        if (tokensItr != tokens.end())
        {
            std::string_view relentlessChanceStr = *(tokensItr++);
            try
            {
                unsigned long relentlessChanceUL = std::stoul(std::string(relentlessChanceStr));
                relentlessChance = static_cast<uint16>(relentlessChanceUL);
            }
            catch (const std::exception& e)
            {
                relentlessChance = 30;
    ...
    static bool HandleSkirmishCommand(ChatHandler* handler, std::vector<std::string_view> args)
    {
        ...
        // Fran: enable skirmish command with bots
        ...
        uint32 team1bots = 0;
        uint32 team2bots = 0;
        std::map<std::string, std::pair<int, uint8>> botClassSpecMap = {
            {"bot1", {1, BOT_SPEC_WARRIOR_ARMS}},
            {"bot2", {1, BOT_SPEC_WARRIOR_FURY}},
            {"bot3", {1, BOT_SPEC_WARRIOR_PROTECTION}},
            {"bot4", {2, BOT_SPEC_PALADIN_HOLY}},
            {"bot5", {2, BOT_SPEC_PALADIN_PROTECTION}},
            {"bot6", {2, BOT_SPEC_PALADIN_RETRIBUTION}},
            {"bot7", {3, BOT_SPEC_HUNTER_BEASTMASTERY}},
            {"bot8", {3, BOT_SPEC_HUNTER_MARKSMANSHIP}},
            {"bot9", {3, BOT_SPEC_HUNTER_SURVIVAL}},
            {"bot10", {4, BOT_SPEC_ROGUE_ASSASINATION}},
            {"bot11", {4, BOT_SPEC_ROGUE_COMBAT}},
            {"bot12", {4, BOT_SPEC_ROGUE_SUBTLETY}},
            {"bot13", {5, BOT_SPEC_PRIEST_DISCIPLINE}},
            {"bot14", {5, BOT_SPEC_PRIEST_HOLY}},
            {"bot15", {5, BOT_SPEC_PRIEST_SHADOW}},
            {"bot16", {6, BOT_SPEC_DK_BLOOD}},
            {"bot17", {6, BOT_SPEC_DK_FROST}},
            {"bot18", {6, BOT_SPEC_DK_UNHOLY}},
            {"bot19", {7, BOT_SPEC_SHAMAN_ELEMENTAL}},
            {"bot20", {7, BOT_SPEC_SHAMAN_ENHANCEMENT}},
            {"bot21", {7, BOT_SPEC_SHAMAN_RESTORATION}},
            {"bot22", {8, BOT_SPEC_MAGE_ARCANE}},
            {"bot23", {8, BOT_SPEC_MAGE_FIRE}},
            {"bot24", {8, BOT_SPEC_MAGE_FROST}},
            {"bot25", {9, BOT_SPEC_WARLOCK_AFFLICTION}},
            {"bot26", {9, BOT_SPEC_WARLOCK_DEMONOLOGY}},
            {"bot27", {9, BOT_SPEC_WARLOCK_DESTRUCTION}},
            {"bot28", {10, BOT_SPEC_DRUID_BALANCE}},
            {"bot29", {10, BOT_SPEC_DRUID_FERAL}},
            {"bot30", {10, BOT_SPEC_DRUID_RESTORATION}},
        };

        std::list<std::string> botsTeam1;
        std::list<std::string> botsTeam2;

        for (; tokensItr != tokens.end(); ++tokensItr)
        {
            last_name = std::string(*tokensItr);
            if (botClassSpecMap.find(last_name) != botClassSpecMap.end()) {
                if (teamCount > 0) {
                    botsTeam1.push_back(last_name);
                    team1bots++;
                }
                else {
                    botsTeam2.push_back(last_name);
                    team2bots++;
                }
                teamCount--;
                continue;
            }
            /*if (last_name.compare("bot1") == 0) {
                team1bots++;
                continue;
            }
            if (last_name.compare("bot2") == 0) {
                team2bots++;
                continue;
            }*/
        ...
        // Fran: enable skirmish command with bots
        bool arenaWithBots = false;
        if (team1bots > 0 || team2bots > 0) {
            arenaWithBots = true;
        }
        uint8 totalTeam1 = team1bots;
        Player* playerTeam1 = NULL;
        Player* playerTeam2 = NULL;
        ...
        bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), players[0]->GetLevel());
        uint32 spawned_a = 0;
        uint32 spawned_h = 0;
        NpcBotRegistry spawned_bots_a;
        NpcBotRegistry spawned_bots_h;

        for (const auto& botName : botsTeam1)
        {
            auto it = botClassSpecMap.find(botName);
            if (it != botClassSpecMap.end())
            {
                bot_ai::SetForcedSpec(it->second.second, true);
                sBotGen->setForcedClass(it->second.first, it->second.second);
                if (!sBotGen->GenerateWanderingBotsToSpawn(1, bg->GetMapId(), HORDE, true, bracketEntry, &spawned_bots_h, spawned_h))
                {
        ...
        for (const auto& botName : botsTeam2)
        {
            auto it = botClassSpecMap.find(botName);
            if (it != botClassSpecMap.end())
            {
                bot_ai::SetForcedSpec(it->second.second, true);
                sBotGen->setForcedClass(it->second.first, it->second.second);
                if (!sBotGen->GenerateWanderingBotsToSpawn(1, bg->GetMapId(), ALLIANCE, true, bracketEntry, &spawned_bots_a, spawned_a))
                {
                    for (NpcBotRegistry const* registry1 : { &spawned_bots_a, &spawned_bots_h })
                        for (Creature const* bot : *registry1)
                            BotDataMgr::DespawnWandererBot(bot->GetEntry());
        ...
        BattlegroundQueueTypeId bgqTypeId = sBattlegroundMgr->BGQueueTypeId(bgTypeId, bg->GetArenaType());
        BattlegroundQueue& queue = sBattlegroundMgr->GetBattlegroundQueue(bgqTypeId);
        queue.isSkirmishCommand = true;
        //for (NpcBotRegistry const* registry3 : { &spawned_bots_a, &spawned_bots_h })
        //{
            uint32 seconds_delay = 5;
            for (Creature const* bot : spawned_bots_h)
            {
                bot->GetBotAI()->SetBotCommandState(BOT_COMMAND_STAY);
                bot->GetBotAI()->canUpdate = false;
                if (playerTeam1 != NULL) {
                    bot->GetBotAI()->SetGroup(playerTeam1->GetGroup(), playerTeam1->GetSubGroup());
                }
                const_cast<Creature*>(bot)->SetPvP(true);

                BotBattlegroundEnterEvent* bbe = new BotBattlegroundEnterEvent(players[0]->GetGUID(), bot->GetGUID(), bgqTypeId,
                    botBGJoinEvents[players[0]->GetGUID()].CalculateTime(Milliseconds(uint32(INVITE_ACCEPT_WAIT_TIME) + uint32(BG_START_DELAY_2M)).count()));
                botBGJoinEvents[players[0]->GetGUID()].AddEventAtOffset(bbe, Seconds(seconds_delay));
            }
            for (Creature const* bot : spawned_bots_a)
            {
                bot->GetBotAI()->SetBotCommandState(BOT_COMMAND_STAY);
                bot->GetBotAI()->canUpdate = false;
                if (playerTeam2 != NULL) {
                    bot->GetBotAI()->SetGroup(playerTeam2->GetGroup(), playerTeam2->GetSubGroup());
                }
                const_cast<Creature*>(bot)->SetPvP(true);
